...
А еще можно на основе tailscale развернуть систему мониторинга например zabbix, без использования zabbix proxy сервера так как все хосты будут объденены в одну, общую сеть. То же самое касается DevOps инфраструктуры. Docker swarm, k8s.
